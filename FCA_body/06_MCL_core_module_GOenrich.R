...
#cutoff 0.05 nlink.cut 11 ,ngene 274 ,nedge 2584
#cutoff 0.05 nlink.cut 12 ,ngene 206 ,nedge 1894 
#cutoff 0.05 nlink.cut 13 ,ngene 158 ,nedge 1325 

x=rownames(net1)
x1=gsub('Dmel-','',x)
rownames(net1)=colnames(net1)=x1
...
x=read.table('./brain_snRNA-seq_n15c0.005/cut12_net.txt',header=T)
head(x)
g<-igraph::graph_from_data_frame(x,directed = F)
#https://www.r-graph-gallery.com/247-network-chart-layouts.html
pdf(paste0(out.dir,'cut',cut,'_net_Rplot.pdf'),useDingbats = T,height = 10,width = 10)
plot(g,
     #layout=layout.random,
     #layout=layout.circle,
     #layout=layout.sphere,
     layout=layout.fruchterman.reingold, #layout=layout_with_fr, 
...
wc=mcl(x = net1, addLoops = TRUE, expansion=3,allow1 = F,inflation = 1.5,max.iter = 10000); #sn
table(wc$Cluster)
...
dim(mat) #196 by 196 genes, genes with module membership
mat[mat==T]=1
mat[mat==F]=0
...
pick.cols=c( "#D16103","#FFDB6D","#4E84C4","#52854C")[1:3]
...
pdf(paste0(out.dir,'cut',cut,'_module_GO.pdf'),
    width = 22,height = 6,useDingbats = T)
grid.arrange(grobs = plots3, ncol=3)
dev.off()
