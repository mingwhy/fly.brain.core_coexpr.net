...
dim(tmp)#76 cell cluster
summary(tmp$median.ratio)

sum(tmp$median.ratio<0.02) # 37
...
keep.cell.types=unique(df.all.sum[df.all.sum$type==1,]$cell.type) #39 remain
...
#print(p.median);
print(grid.arrange(arrangeGrob(grobs= plots[1:16],layout_matrix = layout_matrix)))
...
grid.arrange(arrangeGrob(grobs= plots[65:76],layout_matrix = layout_matrix))
dev.off()
