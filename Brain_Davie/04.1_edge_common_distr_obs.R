...
input.folder='./brain_scRNA-seq_n15c0.005_bigScale2/';
output.folder='brain_scRNA-seq_n15c0.005/';
#input.folder='../03.top.perc_net/brain_snRNA-seq_n15c0.005_bigScale2/';
...
subset(pick.gene,commonality==67)$count #613 
pick.edge=subset(common.df[[2]],cutoff==pick.cutoff & commonality!=0)
sum(subset(pick.edge,commonality<=4)$prop)#0.7548281
sum(subset(pick.edge,commonality>=30)$prop)#0.004000115
max(pick.edge$commonality) #64
sum(subset(pick.edge,commonality==36)$count) #427
sum(pick.edge$count) #1942694

#pdf(paste0(output.folder,'/pan_n_gene_edge_commonality.pdf'),width=18,height = 12,useDingbats = T)
pdf(paste0(output.folder,'/pan_n_gene_edge_commonality_top0.05.pdf'),
    width=12,height = 9,useDingbats = T)

#for(pick.cutoff in names(common.genes)){
for(pick.cutoff in c('0.05')){
  ...
  p1<-ggplot(x,aes(x=commonality,y=prop,fill=factor(cutoff)))+
    ...
    geom_text(label=x$count,hjust=-0.1,vjust=0.4,size=3,angle=90)+
  ...
  #pick.gene$prop=round(pick.gene$prop,4)
  tmp=data.frame(commonality=my.level)
  ...
  p2<-ggplot(x,aes(x=commonality,y=prop,fill=factor(cutoff)))+
    ...
    scale_x_discrete(expand = expansion(mult = c(0.02, 0.02)),
                     breaks=seq(1,67,2),
                     labels=seq(1,67,2))+
    scale_y_continuous(trans = scales::pseudo_log_trans(1e-04, 10),
                       breaks=c(0,1e-04,1e-03,0.01, 0.1, 1),
