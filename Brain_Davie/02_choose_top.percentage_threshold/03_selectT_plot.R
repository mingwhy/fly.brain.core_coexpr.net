...
if(!file.exists(out.file)){
  ...
  for(cell.type in unique(df.all.sum$cell.type)){
    ...
    plots[[cell.type]]<- ggplot(x,aes(x=threshold,y=value))+
      geom_point()+
      geom_smooth()+
      ...
      ggtitle(cell.type)+theme(axis.text.x = element_text(size=9))
  ...
  p.median<- ggplot(df.all.sum,aes(x=threshold,y=median.ratio,group=cell.type))+
           ...
           ggtitle("Threshold choice for gene co-expression networks") +
           #scale_y_continuous("Density-adjusted consistency") +
           scale_y_continuous("Signal to noise ratio")+ #scale_y_log10()+
...
pdf('sc_median.score_over.thresholds.pdf',useDingbats = T,width = 12)
...
keep.cell.types=unique(df.all.sum[df.all.sum$type==1,]$cell.type) #68 remain
length(keep.cell.types)
#write.table(keep.cell.types,'sc_keep.cell.types.txt',quote=F,row.names = F,col.names = F)

####
which(names(plots)=='male_5')
#layout_matrix = matrix(1:16,4,4,byrow = T)
layout_matrix = matrix(1:35,7,5,byrow = T)

pdf('sc_bigscale_rep10.pdf',useDingbats = T,width = 14,height = 14)
...
grid.arrange(arrangeGrob(grobs= plots[1:35],layout_matrix = layout_matrix))
grid.arrange(arrangeGrob(grobs= plots[36:68],layout_matrix = layout_matrix))
dev.off()
