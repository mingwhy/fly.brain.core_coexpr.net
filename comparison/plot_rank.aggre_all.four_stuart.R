...
for(dat.name in datasets){
  if(dat.name=='body, Li et al. 2022'){
    ## FCA.body
    out.dir='~/Documents/fly.brain_coexpr.net_2022.06/FCA_body/05.rank_aggregation_stuart//RAA_output/';
    (files=Sys.glob(paste0(out.dir,'/*sample*edges.rds')));
    length(files)
    ncell.cluster=31
  }else if(dat.name=='head, Li et al. 2022'){
    ## FCA.head
    out.dir='~/Documents/fly.brain_coexpr.net_2022.06/FCA_head/05.rank_aggregation_stuart/RAA_output/';
    (files=Sys.glob(paste0(out.dir,'/*sample*edges.rds')));
    length(files)
    ncell.cluster=39
...
names(all.out)
#saveRDS(all.out,'plot_rank.aggre_all.four_stuart.rds')

##
#all.out=readRDS('plot_rank.aggre_all.four_stuart.rds')
...
df$edge.commonality.group=factor(df$edge.commonality.group,levels=unique(as.numeric(as.character(df$edge.commonality.group))))
...
p2<-ggplot(df,aes(x=edge.commonality.group,y=log_fdr,col=dataset,group=dataset))+geom_point()+
  ...
  ylab('-log10(FDR)')+xlab('Edge commonality group')

min(df$fdr,na.rm=T)

pdf('plot_rank.aggre_all.four_stuart.pdf',useDingbats = T,width = 12)
print(p1);print(p2);
