...
gene.module=data.table::fread('brain_scRNA-seq_n15c0.005/cut43_net_module_gene.txt')
gene.module=gene.module[!duplicated(gene.module),]
head(gene.module)
dim(gene.module) #189 genes with a module membership

edge.info=data.table::fread('brain_scRNA-seq_n15c0.005/cut43_net_module.txt')
dim(edge.info) #2081 
sum(gene.module$gene %in% unique(c(edge.info$from,edge.info$to))) #189
...
(files=Sys.glob(paste0('../02.coexpr_net/brain_scRNA-seq_n15c0.005/bigScale2_coexpr/*.rds')))
length(files) # 67
