...
#cutoff 0.05 nlink.cut 33 ,ngene 88 ,nedge 323, 47 clusters
#cutoff 0.05 nlink.cut 37 ,ngene 65 ,nedge 163, 54 clusters
...
x=read.table('common.gene_CGgenes2symbols.txt')
tmp=x[match(rownames(net1),x[,1]),]
sum(tmp[,1]==rownames(net1));dim(net1)
rownames(net1)=colnames(net1)=tmp[,3]
g<-igraph::graph_from_adjacency_matrix(net1,mode='undirected',weighted = NULL,diag=F) 
df.g<-igraph::as_data_frame(g)
df.g$edge=1
data.table::fwrite(df.g,file=paste0(out.dir,'cut',cut,'_net.txt'),quote=F,sep='\t',row.names = F)

x=read.table('./brain_scRNA-seq_n15c0.005/cytospace/cut37_net2.txt',header=T)
head(x)
g<-igraph::graph_from_data_frame(x,directed = F)
#https://www.r-graph-gallery.com/247-network-chart-layouts.html
pdf(paste0(out.dir,'cut',cut,'_net.pdf'),useDingbats = T,height = 10,width = 10)
plot(g,
