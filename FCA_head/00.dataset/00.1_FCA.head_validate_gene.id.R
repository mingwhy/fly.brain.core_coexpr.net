...
#(files=Sys.glob('*FCA_*/*filtered.rds'))#17 files, something wrong with testis dataset
(files=Sys.glob('*filtered.rds'))#17 files, something wrong with testis dataset

for(file in files){
  cat('begin file',file,'\n')
  dat=readRDS(file) #12616 features across 100267 samples within 1 assay
  genes=rownames(dat)
  ...
  dat1 #12602 features across 52947 samples
